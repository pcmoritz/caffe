...
template <typename Dtype>
void PowerLayer<Dtype>::RvForward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    R_top_data[i] = R_bottom_data[i] * diff_scale_ * top_data[i] / (shift_ + scale_ * bottom_data[i]);
  }
}

template <typename Dtype>
void PowerLayer<Dtype>::RGvBackward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      R_bottom_diff[i] = R_top_diff[i] * diff_scale_ * top_data[i] / (shift_ + scale_ * bottom_data[i]);
    }
  }
}

template <typename Dtype>
void PowerLayer<Dtype>::RHvBackward_cpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, vector<Blob<Dtype>*>& bottom) {
  if (propagate_down[0]) {
    const Dtype* top_data = top[0]->cpu_data();
    const Dtype* top_diff = top[0]->cpu_diff();
    ...
    for (int i = 0; i < count; ++i) {
      ...
      // TODO: This is most likely slow
      R_bottom_diff[i] = R_top_diff[i]
          * diff_scale_ * top_data[i] / (shift_ + scale_ * bottom_data[i])
          + R_top_data[i] * scale_ * scale_ * (power_ - 1) * power_ * pow(shift_ + scale_ * bottom_data[i], power_ - 2);
    }
  }
}
